...
if ($_GET['id'] > 0 && !empty($_GET['id'])) 
{
	...
	$single_prod_data = $mysqli->query("select imdid, type,images from images where imdid = '".$_GET['id']."' limit 1  ");

	if ($single_prod_data->num_rows == 1) 
	{
		$single_row = $single_prod_data->fetch_assoc();
		$imdid = $single_row['imdid'];
...
<div class="col-lg-12 order-1 order-lg-2">
							...
							<div class="tab-pane tab-pane-navigation active" id="formsStyleDefault">
								...
								<h4 class="mb-3"><?= !empty($imdid) ? 'Update Upload Image' : 'Add Upload Image ' ?></h4>
								<div class="card mb-4">
									<div class="card-body">
										<div class="row">
											<div class="col">
											  	<form class="contact-form" action="add_images.php" method="POST" class="needs-validation" novalidate="novalidate" enctype="multipart/form-data">
													...
													<div class="row">
														<div class="form-group col-lg-6">
															...
															<select class="form-control" name="type">
																...
																<option <?= $single_row['type'] == 'Slider' ? 'selected' : '' ?> value="Slider">Slider (1920 x 672)</option>
																<option <?= $single_row['type'] == 'Clients' ? 'selected' : '' ?> value="Clients">Clients (390 x 73)</option>
																<option <?= $single_row['type'] == 'Home About' ? 'selected' : '' ?> value="Home About">Home About (318 x 288)</option>
																<option <?= $single_row['type'] == 'Home section 1' ? 'selected' : '' ?> value="Home section 1">Home section 1 (145 x 145)</option>
																<option <?= $single_row['type'] == 'Home section 2' ? 'selected' : '' ?> value="Home section 2">Home section 2 (145 x 145)</option>
																<option <?= $single_row['type'] == 'Home section 3' ? 'selected' : '' ?> value="Home section 3">Home section 3 (145 x 145)</option>
																<option <?= $single_row['type'] == 'Home section 4' ? 'selected' : '' ?> value="Home section 4">Home section 4 (338 x 338)</option>
																<option <?= $single_row['type'] == 'About Page' ? 'selected' : '' ?> value="About Page">About Page (1170 x 780  )</option>
															</select>
														</div>
														<div class="form-group col-lg-6">
															<label class="form-label mb-1 text-2"> Images</label>
															<input type="file" value="" data-msg-required="Multiple Image" data-msg-email="Please enter a valid email address." maxlength="100" class="form-control text-3 h-auto py-2" name="images[]" multiple required="">
															<?= $multiple_images; ?>
                                                        </div>
													</div>
													
													<?php if (!empty($imdid) && $imdid > 0) {?>
														<input type="hidden" name="imdid" value="<?= !empty($imdid) && $imdid > 0 ? $single_row['imdid']  : '' ?>" />
														<input type="hidden" name="opt" value="edit" />
													<?php } ?>
													<div class="row">
														<div class="form-group col">
															<input type="submit" value=" <?= !empty($imdid) && $imdid > 0 ? 'Update Images' : 'Add Images' ?>" name="submit" class="btn btn-primary" data-loading-text="Loading...">
														</div>
													</div>
												</form>
											</div>
